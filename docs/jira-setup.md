# BatonBot ↔ Jira Setup Guide

**Time required: ~15 minutes.** This connects a Jira project to a BatonBot project so new tickets automatically become cards — and urgent ones auto-run.

## How it works

BatonBot **polls** Jira Cloud's REST API on an interval (default 60s). No webhooks, no public URL, no tunnel — outbound HTTPS only, so it works from any laptop behind any firewall. The machine running BatonBot must be on for polling to happen.
...
BatonBot keeps the ticket reporter in the loop without them ever opening BatonBot:

| Event | Comment |
|---|---|
| Ticket imported | 🤖 BatonBot picked up this ticket… |
| Card moved PENDING → QUEUE (or back) | 📋 BatonBot moved this ticket… |
| Task completed | ✅ BatonBot completed this. \<summary\> — and the ticket **auto-transitions to Done** (v3.4.1, can be disabled) |
| Task failed | ❌ BatonBot couldn't complete this: \<reason\> — ticket stays put |
...
2. Click **Create API token**, name it `batonbot`, copy the token.

> The token authenticates as *your* Jira account. BatonBot needs read access to the project's issues and (optionally) permission to add comments.

## Step 2 — Configure BatonBot

1. Open BatonBot → select your project → click the **✎** (edit) button in the context bar.
...
3. The ticket appears as a card in **PENDING** on the BatonBot board, and BatonBot posts a comment on the Jira issue.
...
For teams that triage in Jira: create a dedicated Jira account (e.g. `batonbot@yourco.com`), set it as the **Bot Account Email**, and have engineers explicitly assign tickets to that account when they want BatonBot to take them. Unassigned tickets still import; human-assigned tickets are always skipped.
...
2. **Token stored in plaintext.** The API token lives in `prompts.json` on the machine running BatonBot. Don't commit that file; rotate the token at id.atlassian.com if compromised.
...
5. **Single-instance assumption.** Two BatonBot instances polling the same Jira project would each import every ticket (dedup state is per-instance, in each machine's prompts.json).
...
7. **One-way channel.** Tasks created directly on the BatonBot board are NOT mirrored to Jira as new issues. Only Jira-originated cards get lifecycle comments. (Two-way sync is a candidate for the Open Substrate release.)

## Security notes

- The API token is stored in `prompts.json` on the machine running BatonBot. Don't commit that file.
